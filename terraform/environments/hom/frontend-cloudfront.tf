...
resource "aws_cloudfront_distribution" "main" {
  ...
  origin {
    # Único ajuste em relação ao monorepo: bucket lido por data source
    # (frontend-data.tf), não por resource — o bucket em si fica no
    # monorepo. Mesmo valor que aws_s3_bucket.frontend.bucket_regional_domain_name
    # retornava, sem diff no plan (ver frontend-data.tf).
