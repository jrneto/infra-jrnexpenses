...
resource "aws_cognito_user_pool" "main" {
  ...
  # cadastro (FEAT-19) — ver
  # backend/infra/terraform/environments/hom/lambda-account-trigger.tf
  # no monorepo. Personaliza assunto/corpo dos e-mails de
  # SignUp/ResendCode/ForgotPassword com os templates HTML de marca
  # própria (FEAT-34) — ver
  # backend/infra/terraform/environments/hom/lambda-custom-message-trigger.tf
  # no monorepo. As duas Lambdas ficam no monorepo (workload); esta
  # config só as lê por nome, nunca por state (ver backend-data.tf).
  lambda_config {
    post_confirmation = data.aws_lambda_function.account_trigger.arn
    custom_message    = data.aws_lambda_function.custom_message_trigger.arn
  }
  ...
  # identidade estar VERIFICADA (não só criada) — ver backend-ses.tf.
  email_configuration {
...
resource "aws_cognito_user_pool_client" "spa" {
  ...
  # no monorepo). Trocar quando existir um frontend de homologação real.
